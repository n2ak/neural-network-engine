...
__global__ void matmul_batched_kernel(
    ...
    int strideB_batch, int strideB_k, int strideB_n,
    ...
    for (int t = 0; t < (K + TILE_DIM - 1) / TILE_DIM; t++) {
        ...
        if (tiledRowB < K && tiledColB < N)
            Bs[threadIdx.y][threadIdx.x] =
                B[batch * strideB_batch + tiledRowB * strideB_k + tiledColB * strideB_n];
        else
