...
__device__ inline T __exec_op(T x,int uop){
    switch(uop){
        ...
        case _UOP_LOG2: return log2(x);
    }
    return 0; // should never reach
}

template<typename I,typename O>
__global__ void unary_op_kernel(
    const I* A, const int* stride_A,
...
template<typename I,typename O>
void unary_op(
    const I *d_a, const int* stride_A,
    ...
    unary_op_kernel<<<gridSize, blockSize>>>(
        d_a, d_stride_A,
        d_c, d_stride_C,
        d_shape,
        ndim,
