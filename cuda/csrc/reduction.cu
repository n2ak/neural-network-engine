...
template <typename I, typename O>
__global__ void
reduction_axis_kernel(const I *input, O *output, const int *in_shape,
                      const int *in_strides, const int *out_shape,
                      const int *out_strides, const int *axes, int nreduce_axes,
                      int ndim_in, int ndim_out, int out_size,
                      int reduction_op) {
...
template <typename I, typename O>
__global__ void reduction_kernel(const I *A, O *C, int totalSize,
                                 int reduction_op) {
  ...
  if (idx < totalSize)
    ...
    shared[threadIdx.x] = __default_value<O>(reduction_op); // TODO
  ...
  for (int s = blockDim.x / 2; s > 0; s /= 2) { // blockDim.x = BLOCK_SIZE
    if (threadIdx.x < s) {
      shared[threadIdx.x] =
          __reduce(shared[threadIdx.x], shared[threadIdx.x + s], reduction_op);
    }
    __syncthreads();
  }
...
template <typename I, typename O>
void reduction_axis(const I *A, const int *stride_A, const int *shape_A, O *C,
                    const int *stride_C, const int *shape_C, const int *axis,
                    int ndim_A, int ndim_C, int nAxis, int reduction_op) {
  int totalSize = _size(shape_C, ndim_C);
  int *d_axis;
  shapeToDevice(axis, &d_axis, ndim_A);
  ...
  int *d_stride_C;
  shapeToDevice(stride_C, &d_stride_C, ndim_A);

  int blockSize = 256;
  int gridSize = (totalSize + blockSize - 1) / blockSize;
  reduction_axis_kernel<<<gridSize, blockSize>>>(
      A, C, d_shape_A, d_stride_A, d_shape_C, d_stride_C, d_axis, nAxis, ndim_A,
      ndim_C, totalSize, reduction_op);
}

template <typename I, typename O>
void reduction(const I *A, O *C, int totalSize, int reduction_op) {
