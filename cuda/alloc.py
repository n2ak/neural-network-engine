...
class Buffer:
    @property
    def nbytes(self):
        return get_nbytes(self.shape, self.stride, self.dtype)
    ...
    @classmethod
    def new(
        cls, shape: tuple[int, ...], stride: tuple[int, ...], dtype: np.typing.DTypeLike
    ):
        ptr = c_void_p()
        assert_cuda_error(
            CudaAllocator.alloc.call(byref(ptr), get_nbytes(shape, stride, dtype))
        )
        buff = Buffer(ptr, shape=shape, stride=stride, dtype=dtype)
        return buff
...
class CudaAllocator:
    ...
    alloc = _CUDA.define_function("cudaMalloc", [POINTER(c_void_p), c_size_t], c_int)
    _free = _CUDA.define_function("cudaFree", [c_void_p], c_int)
    memcpy = _CUDA.define_function(
        "cudaMemcpy", [c_void_p, c_void_p, c_size_t, c_int], c_int
    )
    get_last_error = _CUDA.define_function("cudaGetLastError", [], c_int)
    sync = _CUDA.define_function("cudaDeviceSynchronize", [], c_int)
    ...
    def alloc_empty(cls, shape, stride, dtype: np.typing.DTypeLike) -> Buffer:
        assert (
            len(shape) <= MAX_DIMS
        ), f"Only tensors of up to {MAX_DIMS} dimensions can be allocated, requested: {len(shape)}"
    ...
    def to_cuda(cls, host_array: np.typing.NDArray):
        buffer = cls.alloc_empty(
            host_array.shape,
            [s // host_array.itemsize for s in host_array.strides],
            host_array.dtype,
        )
        err = cls.memcpy.call(
            buffer.ptr,
    ...
    def from_cuda(cls, buffer: Buffer, new_shape=None, new_stride=None):
        ...
        arr = np.lib.stride_tricks.as_strided(
            arr,
            shape=new_shape,
            strides=[s * arr.itemsize for s in new_stride],  # type: ignore
        )
        return arr
...
def get_nbytes(
    shape: tuple[int, ...], stride: tuple[int, ...], dtype: np.typing.DTypeLike
):
